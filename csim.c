 ...
 *
 * Which replacement policy is implemented?  ___LRU (Least Recently Used)________
...
// updated: record the most recent time(LRU)
...
void init_cache()
{
    ...
    if (cache == NULL)
    {
        fprintf(stderr, "malloc failed for cache sets\n");
        exit(1);
    }
...
void access_data(mem_addr_t addr)
{
    ...
    mem_addr_t set_index = (addr >> b) & ((1ULL << s) - 1ULL);
    ...
    for (int i = 0; i < E; i++)
    {
        ...
        if (line->valid && line->last_used < min_used)
        {
    ...
    {
        hit_cnt++;
        set[hit_index].last_used = access_time;
        return;
    }
    ...
    if (empty_index != -1)
    {
        cache_line_t *line = &set[empty_index];
        line->valid = 1;
        line->tag = tag;
        line->last_used = access_time;
    }
    else
    {
        evict_cnt++;
        cache_line_t *line = &set[lru_index];
        line->tag = tag;
        line->last_used = access_time;
    }
}
